...
if is_windows; then
  PIP_FILE_PREFIX="bazel-bin/tf_ops/build_pip_pkg.exe.runfiles/mecab-bind/"
else
  PIP_FILE_PREFIX="bazel-bin/tf_ops/build_pip_pkg.runfiles/mecab-bind/"
fi
...
function main() {
  ...
  cp ${PIP_FILE_PREFIX}tf_ops/MANIFEST.in "${TMPDIR}"
  rsync -avm -L --exclude='*_test.py' ${PIP_FILE_PREFIX}tf_ops/mecab_tf "${TMPDIR}"

  # copy libmecab.so.2 if linux
  LIBMECAB_SO_FILE=`find ${PIP_FILE_PREFIX} -name libmecab.so.2`
  if [[ -n "${LIBMECAB_SO_FILE}" ]]; then
    echo "Copy ${LIBMECAB_SO_FILE}"
    cp ${LIBMECAB_SO_FILE} "${TMPDIR}/mecab_tf/python/ops"
  fi
